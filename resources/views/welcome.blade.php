...
<body class="antialiased h-full">
    <div class="fixed w-full backdrop-blur-lg bg-white/60 dark:bg-zinc-800/80 z-[51]">
        <nav class="px-5 sm:px-12 py-5 sm:py-8 flex items-center whitespace-nowrap">
            <div class="block sm:flex mx-auto sm:mr-20">
                <h1 class="font-black text-2xl dark:text-white">
            ...
            <div class="hidden sm:flex justify-between w-full items-center">
                <ul class="flex">
                    <li class="mr-8">
                        <a href="{{ route('view_menus') }}" class="text-sm dark:text-white font-bold">MENU</a>
                    </li>
                    <li class="mr-8">
                        <a href="{{ route('about_us') }}" class="text-sm dark:text-white font-bold">ABOUT US</a>
                    </li>
                </ul>
                <ul class="flex">
                    @auth
                        @role('Admin|Chef|Cashier|Waiter')
                            <li class="mr-8">
                                <a href="{{ route('dashboard') }}" class="dark:text-white hover:underline">
                                    Dashboard
                                </a>
                            </li>
                        @endrole
                        <li class="mr-8">
                            <a href="{{ route('cart') }}">
                                <i class='bx bxs-cart dark:text-white'></i>
                            </a>
                        </li>
                        <li class="mr-8">
                            <a href="{{ route('profile') }}">
                                <i class='bx bxs-user-circle dark:text-white'></i>
                            </a>
                        </li>
                        <li class="mr-8">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit">
                                    <i class='bx bx-log-out dark:text-white'></i>
                                </button>
                            </form>
                        </li>
                    @else
    ...
    {{-- Hero --}}
    <div class="min-h-screen bg-slate-100 dark:bg-zinc-800 relative">
        @yield('content')
    </div>

    {{-- Footer --}}
    <div class="bg-yellow-50 relative">
        <footer class="p-10 pb-12 bg-white sm:p-6 dark:bg-zinc-800">
            ...
            <div class="pb-6 sm:p-0 sm:flex sm:items-center sm:justify-between">
    ...
    <div
        class="fixed sm:hidden justify-between w-full items-center z-[55] bottom-0 bg-white/60 dark:bg-zinc-800/70 backdrop-blur-md">
        <ul class="flex justify-between p-3 max-w-xs mx-auto">
            <li class="text-center px-3">
                <a href="{{ route('view_menus') }}" class="text-sm font-bold dark:text-white">
                    <i class='bx bxs-food-menu inline-block'></i>
                    <span class="block text-xs">Menu</span>
                </a>
            </li>
            <li class="text-center px-3">
                <a href="{{ route('cart') }}" class=" dark:text-white">
            ...
            <li class="text-center px-3">
                ...
                @endphp
                <a href="{{ $url }}" class="dark:text-white">
    ...
    <script>
        ...
        let get_theme_local_storage = localStorage.getItem('theme_mode');
        let light_btn = document.getElementById('light')
        let dark_btn = document.getElementById('dark')

        if (get_theme_local_storage) {
            if (get_theme_local_storage == "true") {
